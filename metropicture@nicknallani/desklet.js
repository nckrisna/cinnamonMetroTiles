...
const uuid      = "metropicture@nicknallani";
...
MetroTileDesklet.prototype = {
    ...
    this.settings.bindProperty(Settings.BindingDirection.IN, "title-font"   , "fontsize"   , this.on_setting_changed);
     ...
     try{    
		this.appCmd = this.appCmd.replace("file://","");
				this.icon = this.icon.replace("file://","");
	...
	refreshDecoration: function() {
	   try{
        ...
        this._container.set_style("background-color:"+this.bgcolor+";background-image: linear-gradient("+this.bgcolor+", yellow);height:"+this.height+"px;width:"+this.width+"px;border:"+this.btnbrdrwdt+"px solid "+this.btnbrdrclr+";-moz-border-radius:"+this.radius+"px "+this.radius+"px "+this.radius+"px "+this.radius+"px;-webkit-border-radius:"+this.radius+"px "+this.radius+"px "+this.radius+"px "+this.radius+"px;border-radius:"+this.radius+"px "+this.radius+"px "+this.radius+"px "+this.radius+"px;display:inline-block;padding:"+this.padding+"px; background-image:url('"+this.icon+"'); background-repeat: no-repeat; background-size: "+this.width+"px "+this.height+"px;");           
        ...
        this._btn.set_style('border:2px solid '+this.bgcolor+';vertical-align:middle;background-position: center;  display:block;background-color:'+this.bgcolor+'height:'+this.height+'px;width:'+this.width+'px;background-image:url('+this.icon+');');
